...
Pane {
    ...
    // Detección de teclado físico - se activa si no hay teclado disponible
    property bool hasPhysicalKeyboard: true
    property bool showVirtualKeyboardOnFocus: false
    
    Component.onCompleted: {
        // Intenta detectar si hay teclado físico disponible
        // Si Qt.inputMethod.keyboardRectangle está disponible, hay teclado físico
        try {
            hasPhysicalKeyboard = Qt.inputMethod.keyboardRectangle.width > 0
        } catch(e) {
            // Por defecto asumimos que hay teclado
