...
validate_json_structure() {
    ...
    local theme_keys=$(jq -r 'keys | .[]' "$schema_file" 2>/dev/null)
    local file_keys=$(jq -r 'keys | .[]' "$json_file" 2>/dev/null)
    
    if [ $? -ne 0 ]; then
        return 1
    fi
    
    # Verificar si todas las claves requeridas están presentes
    while IFS= read -r key; do
        ...
        local schema_nested=$(jq -r ".${key} | keys | .[]" "$schema_file" 2>/dev/null)
        if [ -n "$schema_nested" ]; then
            local file_nested=$(jq -r ".${key} | keys | .[]" "$json_file" 2>/dev/null)
            while IFS= read -r nested_key; do
                if ! echo "$file_nested" | grep -q "^${nested_key}$"; then
                    return 1
                fi
            done <<< "$schema_nested"
        fi
...
init_paths() {
    ...
    if [ -z "$THEME_SCHEMA" ]; then
        THEME_SCHEMA="$CONFIG_DIR/theme.json"
    fi
...
EOF
    ...
    # Crear esquema theme.json
    print_info "Creating theme.json schema"
    cat > "$THEME_SCHEMA" << 'EOF'
{
  "colors": {
    "accent": "",
    "info": "",
...
list_themes() {
    ...
    if [ ! -f "$THEME_SCHEMA" ]; then
        print_error "Theme schema not found: $THEME_SCHEMA"
        print_info "Run 'theme-selector.sh init' first"
        return 1
    fi
    ...
    for theme_file in "$THEMES_DIR"/*.json; do
        ...
        # Omitir el archivo de esquema theme.json
        ...
        if validate_json_structure "$theme_file" "$THEME_SCHEMA"; then
            echo -e "${GREEN}✓${NC} $theme_name"
        else
            echo -e "${RED}✗${NC} $theme_name ${RED}(invalid structure)${NC}"
        fi
    done
...
generate_themes() {
    ...
    # Validar estructura del tema
    if ! validate_json_structure "$theme_file" "$THEME_SCHEMA"; then
        print_error "Theme file has invalid structure"
        return 1
    fi
    ...
    for source in "${sources_to_process[@]}"; do
        ...
        for ((i=0; i<apps_count; i++)); do
            ...
            while [[ "$content" =~ \[\[([a-zA-Z0-9._:-]+)\]\] ]]; do
                ...
                if [ $? -ne 0 ]; then
                    print_error "Failed to resolve: $placeholder"
                    continue 2
                fi
